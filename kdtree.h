...
class KDTree
{
  ...
  public:
    ...
    void constructHelper(vector<Point<Dim>>& newPoints, int left, int right, int dimension, KDTreeNode*& subroot);

    int partition(vector<Point<Dim>>& list, int left, int right, size_t pivotIndex, int dimension);
    Point<Dim> select(vector<Point<Dim>>& list, int left, int right, size_t k, int dimension);
  ...
  private:
     ...
     */
    void findNN(KDTreeNode* child, const Point<Dim>& query, int dimension, Point<Dim>& best) const;

    double getDistance(const Point<Dim> &q, const Point<Dim> &s, int dim) const;
